...
#'
#' @importFrom "ggplot2" "ggplot" "geom_bar" "aes" "aes_string"
#' "theme_minimal" "theme" "labs" "element_text"
#'
#' @param teamdata_matchup A list of two dataframes, each containing the
...
matchup_chart <- function(teamdata_matchup) {
  team1_statistics <-
    teamdata_matchup$list_data$team1_year1[, c(16, 18, 20, 22)]
  team2_statistics <-
    teamdata_matchup$list_data$team2_year2[, c(16, 18, 20, 22)]
  ...
  combined_stats <- rbind(
    data.frame(Team = "Team1", team1_statistics),
    data.frame(Team = "Team2", team2_statistics)
  )

  combined_stats_long <- tidyr::gather(
    combined_stats,
    "Statistic", "Value", "EFG.",
    "FTR", "TOV.", "OREB."
  )
  ...
  ggplot(combined_stats_long, aes_string(
    x = "Statistic",
    y = "Value", fill = "Team"
  )) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(
      title = "Comparison of Team Statistics",
      x = "Statistic",
      y = "Value"
    ) +
    theme_minimal() +
